  ...
  for (let x = 0; x < columns; x++) {
    for (let y = 0; y < rows; y++) {
      ...
      if (color) {
        ...
        x1 =
          (noise.simplex3(x / config.zoom, y / config.zoom, noiseZ) *
            config.fieldForce) /
          20;
        y1 =
          (noise.simplex3(
...
function drawText(callback) {
  ...
  logo.src = sw > 768 ? "./home-logo-500.png" : "./home-logo.png";
  logo.onload = () => {
...
function drawParticles() {
  ctx.strokeStyle = `rgba(146, 0, 80, ${colorConfig.particleOpacity})`;
  let x;
  let y;
  particles.forEach(p => {
    x = p.pos.x / size;
